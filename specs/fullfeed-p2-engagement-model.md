...
4. **Privilege is binary today.** `is_current_user_admin()` = `profiles.is_admin` (Q: function body); 51 RLS policies call it, 2 read `is_staff`, 33 public functions reference admin/staff. There is no moderator tier. The only elevation path is `supabase/functions/claim-facilitator-admin/index.ts:145` (`update({ is_admin: true, user_role: 'facilitator' })`, HMAC code, rate-limited, anonymous-blocked at `:72`).
...
- **Admin elevation**: `claim-facilitator-admin` (grant-only, HMAC-peppered code, rate limit via `admin_code_redemptions`).
...
- **`profiles.badge_summary jsonb`**: `{families:{<fam>:{count,level}}, badges:{<badge>:{count,level}}, updated_at}` — only level>0 dimensions present. `GRANT SELECT(badge_summary)` to anon+authenticated (profiles has column-only grants). PII columns (full_name etc.) stay private (regression-guarded in smoke 26).

## I1 derivation table (as built)
See the header of the migration for the full table. Kinds: `like, poll_vote, follow, comment, petition_signature, opt_in_completed_provider, opt_in_completed_seeker, conversation_completed, review_received, safety_alert_verified, resource_approved`, plus the reserved `appreciation_gift` (P2.1b, no producer yet). Completed opt-in credits BOTH provider (Helper + family) and seeker (family). Completed conversation credits both participants (Connector + family). Only `review_received` (peer), `safety_alert_verified` (admin) and `resource_approved` (admin) are `verified=true`; P3 admin eligibility reads verified rows only.
...
## I5 unblock flow
`unblock_opt_in(opt_in)` (SECDEF, authenticated-only, guest-guarded): checks the caller authored the post and the opt-in is `declined`, then performs a sanctioned `declined→pending` transition. The P2.0 `enforce_opt_in_transition` graph was extended with a `declined→pending` edge permitted ONLY when the transaction-local GUC `feed.optin_unblock='on'` (set by the RPC) — forward-only holds for every other caller. **Slot-neutral**: the slot is consumed at opt-in and NOT restored on decline (P2.0 behaviour), so the declined seeker still holds it; unblock reuses that same slot. The private `opt_in_declines` marker survives the unblock and is author-only (RLS), never in the seeker's view, a public view, or the WAL.
...
2. **Lost update closed (I3 under concurrency).** `recompute_badge_summary` locks the profile row (`FOR UPDATE`) before rebuilding; two-party events (opt-in/conversation completion) lock both profiles in uuid order (deadlock-safe). Proven by race.sh: two concurrent likes leave both families in the summary, incremental == full.
...
6. **Coverage (all interactions earn).** Added kinds (each once per (actor,target), non-verified, weight 1): `post_created` (family via resource or post chip), `event_checkin` (attendee), `safety_alert_vote`, `message` (once per conversation), `resource_bookmark`, `saved_resource`. Full kind set = 17 + reserved `appreciation_gift`. A one-time idempotent BACKFILL (`SELECT reconcile_engagement(NULL)` at the end of the migration) credits existing prod facts through the same functions; on prod it credits ~1 like, 1 vote, 1 post_created, 1 peer-verified alert, and 0 of the 12 self-approved resources.
...
8. **Unblock = delete-and-requeue (user's words).** `unblock_opt_in` (SECDEF, author-only, declined-only) DELETEs the declined opt-in and restores the slot; the seeker may opt in again themselves via `opt_in_to_post`. It does NOT auto-requeue. The `enforce_opt_in_transition` graph is the P2.0 body verbatim (no `declined→pending` edge, no GUC bypass). A declined opt-in never completed, so it has no reviews to cascade. The private `opt_in_declines` marker survives. UI copy: "Unblock — they can request again".
