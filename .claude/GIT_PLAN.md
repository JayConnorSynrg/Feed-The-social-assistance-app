...
status: pending
type: migration
description: "POST-DEPLOY step for wave H (two PROD writes, read-only prod only until then). STRICT ORDER after the develop squash-merge reaches Vercel state=READY on www.sourcetofeed.com (HTTP 200). (1) Apply supabase/migrations/20261002000000_posts_privilege_least.sql to prod ndtpovonpadugthmcntl via the Management-API SQL endpoint (PAT from apps/web/.env.local; shell env var is stale). It REVOKEs unused table privileges on public.posts from anon (all 7 incl. MAINTAIN — prod is PG17.6) and authenticated (all except INSERT), and DROPs dead policies posts_update_own/posts_delete_own/posts_block_anon_update; REVOKE + DROP POLICY IF EXISTS are idempotent/replay-safe; SELECT/column grants and the realtime publication are untouched. (2) Apply supabase/migrations/20261003000000_restore_realtime_publication.sql — re-adds messages (FULL ROW) + notifications (FULL ROW) + conversations (id,volunteer_id,requester_id) to supabase_realtime via per-table guarded idempotent ADD (NEVER SET TABLE); posts/post_comments/poll_votes untouched. (3) Then run supabase/scripts/backfill-schema-migrations-2026-09.sql (single ON CONFLICT DO NOTHING transaction, 37 rows = 35 out-of-band + 20261002000000 + 20261003000000, since the Management-API applies in steps 1-2 do NOT write the ledger). Apply 20261002000000 and 20261003000000 before the backfill; all are prod writes done only post-merge. Verify post-apply: (a) has_table_privilege matrix — anon INSERT/UPDATE/DELETE/TRUNCATE/REFERENCES/TRIGGER/MAINTAIN all false; authenticated INSERT true, UPDATE/DELETE/TRUNCATE/REFERENCES/TRIGGER/MAINTAIN all false; posts SELECT/column grants unchanged (18 cols, location excluded); policies posts_update_own/posts_delete_own/posts_block_anon_update absent; (b) pg_publication_tables for supabase_realtime = posts(18 cols excl location), post_comments(id,post_id), poll_votes(id,poll_id), messages(full row), notifications(full row), conversations(id,volunteer_id,requester_id); petition_signatures ABSENT; (c) schema_migrations count 97 → 134; (d) prod-smoke 06, 16 and 24 pass, 08 passes as updated (asserts petition_signatures absent), 05 still passes. smoke 24 gate is the ledger row EXISTS version 20261002000000. DO NOT apply as part of the git merge."
