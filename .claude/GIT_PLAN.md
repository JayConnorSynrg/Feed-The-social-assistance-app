...
completed_at: null
verification: null
status_note: "Fix round applied (CHANGES-REQUIRED): smoke-24 now gates on the ledger row (regression fails, no silent skip); MAINTAIN revoke guarded for PG15 local replay; backfill records ALL 36 versions (35 out-of-band + 20261002000000 → ledger 97→133); migration comment adds programs-panel.tsx:71 and DROPs dead policies posts_update_own/posts_delete_own/posts_block_anon_update. tsc 0 errors; vitest non-smoke green; smoke 24 skips today (ledger row absent). Branch cut from origin/develop 64a9fab; no divergence."
```
...
status: pending
type: migration
description: "POST-DEPLOY step for wave H (two PROD writes, read-only prod only until then). STRICT ORDER after the develop squash-merge reaches Vercel state=READY on www.sourcetofeed.com (HTTP 200). (1) Apply supabase/migrations/20261002000000_posts_privilege_least.sql to prod ndtpovonpadugthmcntl via the Management-API SQL endpoint (PAT from apps/web/.env.local; shell env var is stale). It REVOKEs unused table privileges on public.posts from anon (all 7 incl. MAINTAIN — prod is PG17.6) and authenticated (all except INSERT), and DROPs dead policies posts_update_own/posts_delete_own/posts_block_anon_update; REVOKE + DROP POLICY IF EXISTS are idempotent/replay-safe; SELECT/column grants and the realtime publication are untouched. (2) Then run supabase/scripts/backfill-schema-migrations-2026-09.sql (single ON CONFLICT DO NOTHING transaction, 36 rows = 35 out-of-band versions + 20261002000000 itself, since the Management-API apply in step 1 does NOT write the ledger). Step 1 MUST precede step 2 only in intent; both are prod writes done only post-merge. Verify post-apply: (a) has_table_privilege matrix — anon INSERT/UPDATE/DELETE/TRUNCATE/REFERENCES/TRIGGER/MAINTAIN all false; authenticated INSERT true, UPDATE/DELETE/TRUNCATE/REFERENCES/TRIGGER/MAINTAIN all false; posts SELECT/column grants unchanged (18 cols, location excluded); policies posts_update_own/posts_delete_own/posts_block_anon_update absent; (b) schema_migrations count 97 → 133; (c) prod-smoke 24-posts-privilege-least flips skipped→green (its gate is the ledger row EXISTS version 20261002000000, so it also proves the backfill landed). DO NOT apply as part of the git merge. FOLLOW-UP (separate wave, NOT this one): notifications + post_likes remain OFF the realtime WAL after the LIVE W1.3 SET TABLE — a forward migration to re-publish notifications (column-scoped) is deferred pending coordinator instruction."
