...
next_action_id: null
type: branch+commit+pr
description: "feat(feed): P2.1a engagement ledger + profile badges + opt-in unblock (on the P2.0 integrity floor). Migration 20261005000000 (ledger tip 20261004000000) builds: append-only engagement_events ledger (UNIQUE(actor_id,kind,target_id) = award-once-ever; RLS own-read; NO client write policy; not published), user_engagement_counters (family:<k> weighted points / badge:<k> event counts), single-row tunable badge_config (3/10/25, mirrors ranking_config; an UPDATE recomputes all summaries), private per-author opt_in_declines marker (RLS author-only; not published), and profiles.badge_summary jsonb (GRANT SELECT to anon+authenticated — profiles has column-only grants; PII stays private). 18 kinds + reserved appreciation_gift (conversation_completed split into _volunteer/_requester). SECDEF AFTER-ROW triggers credit the ledger; each body is BEGIN/EXCEPTION-wrapped (I7 core-first) and reconcile_engagement() re-derives idempotently (nightly pg_cron, guarded). Guests + NULL actors earn nothing; review_received keys on the anchor, comment on the post; self-verification earns nothing. DECISION A (community-confirmed): A1 — an action whose actor OWNS the target (own like/comment/poll-vote/alert-vote/bookmark/save) earns nothing; A2 — post_created credits the author ONCE at the first outside non-guest engagement (like/comment/opt-in), never at creation, key stable, deleted-before-engagement earns nothing, credit kept after delete (collusion accepted — unverified, never gates privilege). DECISION B (public/private split, classified from live RLS): public credit → profiles.badge_summary; private credit → user_private_badge_summary (owner-only RLS, no client writes, not published, no embed); user_engagement_counters gains a scope column; private credit never touches profiles. Public (actor↔target link readable by others via a granted COLUMN, per live audit): like/poll_vote/follow/comment/post_created/opt_in_completed_provider/conversation_completed_volunteer/resource_approved. Private: petition_signature/event_checkin/safety_alert_vote/message/resource_bookmark/saved_resource/opt_in_completed_seeker/conversation_completed_requester/review_received/safety_alert_verified. Advocate AND Watcher are private badges (safety_alerts.created_by has no client grant while verified_at is granted → a public safety_alert_verified write would unmask the reporter via profiles.updated_at=verified_at). Public summary publishes LEVELS ONLY (no counts), rewritten only on a level change; private summary keeps counts. Every trigger path crediting two profiles calls lock_two_profiles(a,b) in uuid order before recording (cross-like deadlock closed). post_created qualifying engagement = like / non-hidden comment / poll vote / opt-in (live + reconcile identical). Petition creator self-sign earns nothing. reconcile_engagement carries SET lock_timeout=5s. verified=true only for review_received (peer) / safety_alert_verified + resource_approved (admin); P3 admin eligibility reads verified rows. Completed opt-in credits provider (Helper+family) AND seeker (family); completed conversation credits both (Connector+family). I3: badge_summary = recompute(ledger,config), maintained incrementally (recompute locks the profile row FOR NO KEY UPDATE; two-party events lock both profiles in uuid order — deadlock-safe against inbound FK KEY SHARE) + full recompute proven equal; badge_summary carries no updated_at and is rewritten only when its value changes. I5: unblock_opt_in(opt_in) SECDEF authenticated-only — author-only, declined-only; DELETEs the declined opt-in and restores the slot EXACTLY ONCE (rowcount-gated under a post row-lock, so concurrent unblocks cannot overbook); the seeker re-opts themselves via opt_in_to_post; the P2.0 transition graph is untouched (no GUC); the private marker survives, author-only. UI: profile page Community Badges section (pure lib/engagement-badges.ts mapper + empty state, CATEGORY_META colors, lucide icons, WCAG labels) + feed-panel author Unblock button on declined opt-ins + private 'Declined before' marker label. All write helpers SECDEF search_path=public,pg_temp + REVOKE EXECUTE FROM PUBLIC,anon,authenticated (recompute fns -> service_role). types.ts hand-augmented (4 tables + unblock_opt_in + profiles.badge_summary) to mirror post-apply regen. Verified: migration replays twice on PG17 (PG15-safe by construction); 9 behavioural SQL cases; 7 vitest units; tsc 0; build passes; smoke 26 skips pre-apply with 3 mutation-proven probes. DO NOT MERGE — orchestrator validates + authorizes. Migration applied as a SEPARATE post-deploy step (see -postdeploy)."
...
pr: 208
pr_url: https://github.com/JayConnorSynrg/Feed-The-social-assistance-app/pull/208
merge_sha: c09e8edfb3453ae4cdec685a6810655a3a1ee2c9
merged_into: develop
depends_on: feed-fullfeed-p2-0-integrity
created_at: "2026-09-23T14:00:00.000Z"
completed_at: "2026-09-23T18:55:08.000Z"
evidence: "PR #208 squash-merged (c09e8ed). 4 review rounds (all CHANGES-REQUIRED resolved). Verified twice-applied on PG15 15.19 + PG17 17.7: a1a2/conc/conc4/conc5/oracle/state26/race2/dbl/i1/i5/i7/ab all green (0 deadlocks; reporter-unmask leak closed; A1/A2 self-farm=0; public levels-only / private counts). tsc 0; 340 non-smoke vitest + 10 unit; build passes. Vercel READY on c09e8ed (deploy xw8WbieQvrRRWR32q4NegfKdBFsh), site 200."
```
...
next_action_id: null
type: migration
description: "POST-DEPLOY step for wave P2.1a. SCHEMA-FIRST ORDER (the merge auto-deploys, and the new client READS profiles.badge_summary + opt_in_declines and CALLS unblock_opt_in, so the schema must be live before the client). The migration is fully ADDITIVE and non-breaking to the currently-deployed (old) client (new tables/column/RPC + triggers on existing tables; no read-path change for the old client — profiles has column-only grants and every profiles read is an explicit column list or the get_my_profile RPC, neither selecting badge_summary), so applying it before the merge carries zero risk. CORRECTED ORDER: (1) after the adversarial review is CLEAN, apply supabase/migrations/20261005000000_p2_1a_engagement.sql to prod ndtpovonpadugthmcntl via the Management-API SQL endpoint (PAT from apps/web/.env.local; shell env var may be stale) — idempotent/replay-safe (BEGIN/COMMIT; CREATE ... IF NOT EXISTS / CREATE OR REPLACE / ADD COLUMN IF NOT EXISTS / DROP ... IF EXISTS / guarded policy creation; PG15+ safe; ends with SELECT reconcile_engagement(NULL) backfill). (2) VERIFY (catalog + anon surface, below). (3) INSERT the ledger row for 20261005000000 into supabase_migrations.schema_migrations (ON CONFLICT DO NOTHING). (4) Regenerate packages/database/types.ts from prod (npx supabase gen types typescript --project-id ndtpovonpadugthmcntl, PAT in env) and PUSH it to the PR branch (reconciles the hand-added stubs; expect no functional diff). (5) CI green on the branch. (6) MERGE (squash) to develop — this auto-deploys to Vercel. (7) Confirm the develop squash-merge commit reaches Vercel state=READY on www.sourcetofeed.com (HTTP 200). (8) Run prod-smoke 26 (flips skipped->asserting: both tests pass). VERIFY: (a) has_column_privilege(anon,'public.profiles','badge_summary','SELECT')=true, authenticated=true; has_column_privilege(anon,'public.profiles','full_name','SELECT')=false; has_column_privilege(authenticated,'public.profiles','badge_summary','UPDATE')=false; (b) engagement_events relrowsecurity=true, authenticated INSERT=false, own-SELECT policy present, 0 client write policies; (c) badge_config anon SELECT=true, authenticated UPDATE=false; (d) opt_in_declines relrowsecurity=true, anon SELECT=false, author-only SELECT policy; (e) all 16 trg_engagement_* triggers present+enabled, AFTER+ROW, right tgtype bits; (f) record_engagement_event + reconcile_engagement NOT authenticated-executable; (g) unblock_opt_in authenticated EXECUTE=true, anon=false; unblock_opt_in + reconcile_engagement SECDEF + pinned search_path; (h) enforce_opt_in_transition md5(prosrc) unchanged = f3bc362fce1361a5717007ea2f7a88e7 (P2.0 body — unblock deletes); (i) the 5 new tables (incl. user_private_badge_summary) are absent from supabase_realtime; (j) user_private_badge_summary relrowsecurity=true, anon SELECT=false, authenticated INSERT/UPDATE=false, owner-only SELECT policy, 0 client write policies; anon+authenticated EXECUTE on reconcile_engagement + record_engagement_event + credit_post_created = false; (k) source_triggers=16 with resource_opt_ins/trg_engagement_opt_in_insert present (INSERT) and NO posts/trg_engagement_post_created. Expected one-time backfill = 0 PUBLIC ledger rows; the single peer-verified alert credits safety_alert_verified into the PRIVATE ledger for its reporter (count 1, below the level-1 threshold → invisible in the summary); all other prod facts are self-interactions that earn nothing. DO NOT apply as part of the git merge."
project_ref: ndtpovonpadugthmcntl
...
<!-- COMPLETE: feed-fullfeed-p2-1a-badges-surface — P2.1a follow-up capability fix; CLIENT-ONLY (no DB/post-deploy step). Surfaces earned badges in Settings -> Profile because /profile/[username] is unreachable (all prod usernames NULL, no SPA link). Merged PR #209 (squash b6e3ea6); Vercel prod success; site 200. next_action_id=null. -->
...
merge_sha: b6e3ea63167b649f90f00edaa0c04c5d786ef64c
merged_into: develop
depends_on: feed-fullfeed-p2-1a-engagement
created_at: "2026-09-23T15:10:00.000Z"
completed_at: "2026-09-23T15:35:00.000Z"
evidence: "PR #209 squash-merged to develop (b6e3ea6). Review CHANGES-REQUIRED (M1/L1) resolved: on a timed-out read the badge card now shows the error state + Retry (never 'No badges yet') — the fetch/decision logic lives in a node-testable loadMyBadges() that returns error+null-summaries on timeout, mirroring use-follows.ts; the unmount abort is still filtered by the hook's guard. L1: use-my-badges.test.ts (4 cases: success w/ no private row → summaries + no error; timeout → error set; real error → error set + logged; reload re-reads) against a mocked Supabase client — the repo has no React hook renderer (no testing-library/jsdom), so the load core is tested directly, as vault-unlock-timeout.test.ts tests unlockVault. Mutation-checked: reverting the timeout→error return fails the timeout test. Lint gate: the effect was refactored to invoke a useCallback (all setState inside it, per use-notifications.ts) to clear the react-hooks set-state-in-effect error → 0 lint errors. Merge-gate all green: tsc 0; 348 non-smoke vitest pass (+4 hook, +4 badge-view, 10 engagement-badges); build passes (ƒ Proxy Middleware); all 12 CI checks SUCCESS, PR MERGEABLE/CLEAN before merge. Post-merge: develop HEAD b6e3ea6; Vercel production deploy BAMrfnGJxssUF9ZHkjeZqVrRMiz2 => success on the squash SHA; www.sourcetofeed.com => 200. CLIENT-ONLY: no migration, no prod DB write, no post-deploy step. Residual: authenticated ProfileSection DOM not browser-verified (needs a signed-in Supabase session; local sign-up would be a prod auth write, no test credentials) — close by logging in a test account on the dev server → Settings → Profile."
```
