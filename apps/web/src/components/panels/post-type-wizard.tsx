...
// ---------------------------------------------------------------------------
// Photo attach (W1.2) — upload-on-select, shared hook + field
// ---------------------------------------------------------------------------

/**
 * Manages selecting a photo, re-encoding + uploading it via the validating edge
 * function, and exposing the resulting public URL. The upload runs on select so
 * the post insert only carries a URL already validated + stored server-side.
 */
function usePostImagePicker() {
  ...
  return { imageUrl, previewUrl, imageUploading, imageError, fileInputRef, handleFileSelect, clearImage }
}
    ...
    <div>
      ...
      {previewUrl ? (
        ...
        <button
          ...
          className="flex items-center gap-2 rounded-lg border border-dashed border-stone-300 px-3 py-2 text-sm text-stone-600 hover:border-[#4a5d23] hover:text-[#4a5d23] transition-colors"
        >
